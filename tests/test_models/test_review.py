...
class TestReview(unittest.TestCase):
    ...
    def test_create_review_in_database(self):
        """ Test """
        a = "localhost"
        b = "hbnb_test"
        c = "hbnb_test_pwd"
        ...
        db = MySQLdb.connect(host=a, user=b, passwd=c, db=d, port=3306)
        x = db.cursor()
        x.execute("""SELECT id FROM places""")
        id_place = x.fetchall()[0][0]
        db = MySQLdb.connect(host=a, user=b, passwd=c, db=d, port=3306)
        x = db.cursor()
        x.execute("""SELECT COUNT(id) FROM reviews""")
        review_before = x.fetchall()[0][0]
        pm = 'create Review place_id="{}" user_id="{}" text="A,_huge_kitchen"'
        with patch('sys.stdout', new=StringIO()) as f:
            HBNBCommand().onecmd(pm.format(id_place, id_user))
        db = MySQLdb.connect(host=a, user=b, passwd=c, db=d, port=3306)
        x = db.cursor()
        x.execute("""SELECT COUNT(id) FROM reviews""")
        review_after = x.fetchall()[0][0]
